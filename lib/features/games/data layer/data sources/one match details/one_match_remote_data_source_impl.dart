// features/games/data layer/data sources/one match details/one_match_remote_data_source.dart
import 'dart:convert';
...
class OneMatchRemoteDataSourceImpl implements OneMatchRemoteDataSource {
  ...
  final String baseUrl = 'https://www.sofascore.com/api/v1/event/';
  ...
  Future<MatchEventEntity> getMatchEvent(int matchId) async {
    try {
      final response = await client
          .get(Uri.parse('$baseUrl$matchId'))
          .timeout(
            const Duration(seconds: 12),
            onTimeout: () {
              throw const TimeoutException(
                'Match event request timed out after 12 seconds',
              );
            },
          );
      ...
      if (response.statusCode == 200) {
        ...
        throw ServerException(
          'Failed to load match event: ${response.statusCode}',
        );
      }
    } on TimeoutException {
  ...
  Future<MatchStatisticsEntity> getMatchStatistics(int matchId) async {
    try {
      final response = await client
          .get(Uri.parse('$baseUrl$matchId/statistics'))
          .timeout(
            const Duration(seconds: 12),
            onTimeout: () {
              throw const TimeoutException(
                'Match statistics request timed out after 12 seconds',
              );
            },
          );
      ...
      if (response.statusCode == 200) {
        ...
        throw ServerException(
          'Failed to load match statistics: ${response.statusCode}',
        );
      }
    } on TimeoutException {
      rethrow; // Pass the TimeoutException to the repository
